...
object test_CIB {
	...
	// positions of the CIB.t memeber, which is of type List[List[Int]].
	// This requires two recursive functions that call one another
	// (otherwise, we would have to use mutable lists).
	//
	//   INPUT
  ...
  def get_cibs(AA: List[CIB], A: CIB, i0: Int): List[CIB] = {
  	if ((A.isFull) &&  (i0 == A.n))
  		...
  		get_cibs(AA ::: List(A), new CIB(A.n, List(List(i0))), i0+1)
  	else
  ...
  def get_cibs_aux(AA: List[CIB], A: CIB, i0: Int, kstep: Int): List[CIB] = {
	...
	val mylist = get_cibs(List(), new CIB(3, List(List(2))), 3)
                                                  //> mylist  : List[finder.CIB] = List(List(2), List(0, 0), List(2), List(0, 1),
                                                  //|  List(2), List(0, 2), List(2), List(1, 0), List(2), List(1, 1), List(2), Li
                                                  //| st(1, 2), List(2), List(2, 0), List(2), List(2, 1), List(2), List(2, 2))
	mylist(1).cibtable                        //> res8: Array[Array[Int]] = Array(Array(0, 2, 0), Array(2, 1, 1), Array(0, 1,
                                                  //|  2))
